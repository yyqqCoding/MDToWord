...
4. 工具由 Phase Tool Policy 按当前阶段动态注册，未注册工具不存在可执行入口；
...
模型可在一次响应请求多个工具，但只有只读查询可以并行；只读查询与至多一个 Sandbox
Job 可以同批，Worker 仍串行执行。两个 Sandbox、patch 写入与任意其他工具、完成工具与
任意其他工具都在执行前拒绝。详细矩阵以 `repair-agent-loop.md` 为准。
