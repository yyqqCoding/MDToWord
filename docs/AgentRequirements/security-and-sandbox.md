...
生产优先候选为 Cloudflare 注入的单值 `CF-Connecting-IP`，但启用前必须在 Render 完成
一次真实转发验证，不能仅因响应含 `server: cloudflare` 就推断请求头可信。验证至少证明：

1. 同一网络的正常请求与伪造 `CF-Connecting-IP`/`X-Forwarded-For` 请求解析为同一来源；
2. Wi-Fi 与手机流量解析为不同来源；
3. 应用收到的候选值始终是一个合法 IP，而不是逗号分隔的转发链；
4. 临时诊断只记录 HMAC 摘要，验收后关闭，不持久化或输出原始 IP。
