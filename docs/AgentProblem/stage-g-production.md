...
## 问题 16：展示站有调用计数，但沙箱与“无法复现”的工具明细为空

展示站升级为“最近一次产出 PR 的运行”后，最新 PR #2 运行虽然显示 `tool_calls=21`，阶段
详情却提示调用明细未随 Trace 上报；多条 `cannot_reproduce` 运行也存在同样现象。页面改版
只暴露了问题，并未删除数据。对比 Supabase 快照与 Langfuse 当前响应发现，坏快照均在运行
结束约 3 秒后写入，当时子 observation 已可查询，真正的 `feedback-repair-run` 根尚未完成
索引。旧投影器把所有“父节点不在响应中”的子调用当作根，再把这些根从合成根的子列表中
移除，最终固化为 `attempts>1/children=[]`；已有快照又会阻止详情页和手工回填自愈。

### 解决方案

投影器现在只接受稳定命名的 `feedback-repair-run` 根：优先精确匹配 metadata 中的
`run_id`，匹配不到时仍只在命名根中回退。后者用于兼容 UUID 被 masking 误判为电话号码、
出现 `[REDACTED_PHONE]` 的真实记录；不会重新接受任意孤儿调用。根尚未索引时投影返回
`null`，完成回调继续执行原有 4 秒、12 秒重试。

站点同时用数据库 `model_calls + tool_calls` 校验快照：调用计数大于零而合成根没有后代的
快照视为不完整，详情页按需补抓与手工批量入口都会重抓；零调用运行不受影响。一次性回填
先只读盘点 49 条运行，保留 40 条完整快照和 1 条真实零调用快照，只覆盖 7 条可从 Langfuse
明确恢复的坏快照；另 1 条无快照运行在 Langfuse 返回 404，保持缺失而不伪造。回填后再次
盘点只剩该 404 记录，7 条空明细快照均已恢复。修复合并为 `d29a412` 并完成 Vercel 部署；
维护者随后复核最新 PR 运行与多条 `cannot_reproduce` 页面，工具调用和 Sandbox observation
均可正常展开，确认回填数据与后续防复发逻辑同时生效。
