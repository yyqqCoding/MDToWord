...
- **演练修复终态**（2026-08-31 真实运行回归）：`completed + validation.passed=true`
  必须优先显示“候选修复已验证”；`dry_run=true` 只表示未授权创建 PR，不能回退成
  “无法复现”。没有公开 PR Diff 但存在 `validation` 时，代码卡显示“候选补丁”和受信
  `changed_files` 数量，不得显示“无补丁”。完整候选 Diff 继续不从私有 Artifact 上站。
