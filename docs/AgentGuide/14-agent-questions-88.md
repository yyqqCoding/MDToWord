...
Provider统一转换成`timeout`、`rate_limit`或`provider_unavailable`，默认最多额外重试2次，
本地等待约1秒和4秒。429带数值`Retry-After`时会在10秒上限内尊重更长等待。重试耗尽后把
稳定错误码写入运行，不无限等待。
...
Sandbox传输故障使用同一Job ID额外重试一次；没有同权限等价工具时不会自动换工具。安全
失败不会通过降级到Shell解决。
...
Client默认只额外重试一次，并使用相同`job_id`和`Idempotency-Key`。Worker用锁覆盖
“查询结果、执行、保存”，相同请求完成后直接返回持久化结果；相同ID但内容不同返回冲突。
这使网络重放安全，但不能把ID换掉后再称为同一次重试。
